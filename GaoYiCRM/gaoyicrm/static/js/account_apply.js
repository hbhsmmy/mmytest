...
$(document).ready(function() {
    ...
    $.getJSON('/applyDetails/', sendData, function (ret){
        ...
        $("#user-time-2").val(ret.account_date);
        ...
        if(applyStatus == 4){
            ...
            $("#chulirenyuan").attr("disabled",'disabled');
            $("#user-time-1").attr("disabled",'disabled');
            $("#user-time-2").attr("disabled",'disabled');
            $("#base-check").attr("disabled",'disabled');
            $("#card-check").attr("disabled",'disabled');
            $("#test-check").attr("disabled",'disabled');
            $("#asset-check").attr("disabled",'disabled');
            $("#contract-check").attr("disabled",'disabled');
    ...
    $("#base-check").click(function(){
        if($("#base-check").is(':checked')){
            $.getJSON('/checkBaseInfo/', sendData, function (ret){
                if(ret.success == '404'){
                    alert('请根据收到的信息表补充褐色的项目！');
                    $("#base-check").attr("checked",false);
                    if(ret.clientType == '1'){
                        window.open('../account_editor/client='+ret.clientID);
                    }else{
                        window.open('../account_org_editor/client='+ret.clientID);
                    }
                }
            });
        }
    ...
    $("#saveBtn").click(function(){
        ...
        var userTime = $("#user-time-1").val();
        var moneyTime = $("#user-time-2").val();
        var baseCheck = $("#base-check").is(':checked');
        var cardCheck = $("#card-check").is(':checked');
        var testCheck = $("#test-check").is(':checked');
        var assetCheck = $("#asset-check").is(':checked');
        var contractCheck = $("#contract-check").is(':checked');
        var applyCheck = $("#apply-check").is(':checked');
        if(moneyTime!=""){
            if(userBankName == '' || userBankNo == ''){
                alert('请填写开户银行和银行账号信息！')
                return;
            }
        }
...
function waitGlobal2(){
    if(!judgeAuthority(1002)){
        $("#chulirenyuan").css("width","50%");
        $("#user-time-2").css('display','none');
        $("#user-time-2-text").css('display','none');
    }
}